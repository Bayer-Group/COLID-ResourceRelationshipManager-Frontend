...
export class AppComponent implements OnInit, OnDestroy {
  ...
  constructor(
    ...
    this.masterSub.add(this.notificationService.notification$.subscribe(message => {
      this.snackBar.open(message, "Dismiss");
    }));
  }

  ngOnInit() {
    this.masterSub.add(this.authService.isLoggedIn$.subscribe(isAuth => {
      if (isAuth) {
        this.store.dispatch(new FetchMetadata).subscribe();
      }
    }));

    this.masterSub.add(this.authService.isLoggedIn$.pipe(switchMap(isAuth => {
      ...
      if (identity) {
        ...
        return this.store.dispatch(new FetchUser(identity.accountIdentifier, identity.email));
      }
    })).subscribe());

    this.masterSub.add(this.authService.currentEmail$.pipe(
      tap(
        r => {
          if (r) {
    ...
    this.masterSub.add(this.metadataTypes$.subscribe(metadataTypes => {
      if (metadataTypes) {
