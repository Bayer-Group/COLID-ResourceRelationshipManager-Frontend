...
export class AddToLinkEditHistory {
    ...
    constructor(public link: LinkTypeContainer, public action: LinkHistoryAction) { }
}
...
export class GraphLinkingDataState {
    ...
    addLinkableNode({ patchState, getState }: StateContext<GraphLinkingData>, action: AddLinkableNode) {
    ...
    addToLinkHistory({ patchState, getState }: StateContext<GraphLinkingData>, { action, link }: AddToLinkEditHistory) {
        const linkEditHistory = getState().linkEditHistory
        //verify if this same item does not yet exist
        let duplicate: number = linkEditHistory.findIndex(hi =>
            hi.action == action
        ...
        let undo: number = linkEditHistory.findIndex(hi =>
            hi.linkType.value == link.linkType.value
            && hi.source.uri == link.source.uri
            && hi.target.uri == link.target.uri
        );
        if (undo > -1) {
            if (action != linkEditHistory[undo].action) {
                let history: LinkEditHistory[] = JSON.parse(JSON.stringify(linkEditHistory));
            ...
            patchState({
                linkEditHistory: [...linkEditHistory, { action, source: link.source, target: link.target, linkType: link.linkType }]
            })
        }
    }
    ...
    removeFromHistory({ patchState, getState }: StateContext<GraphLinkingData>, { item }: RemoveFromHistory) {
        //find item to delete
        const linkEditHistory = getState().linkEditHistory;
        let cleanedItems: LinkEditHistory[] = JSON.parse(JSON.stringify(linkEditHistory));
