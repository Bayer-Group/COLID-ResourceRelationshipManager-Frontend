import { Component, OnInit, Input, Output, ChangeDetectionStrategy, forwardRef } from '@angular/core';
import { AggregationBucket } from '../../../shared/aggregation-bucket';
import { Store } from '@ngxs/store';
import { Aggregation, AggregationType } from '../../../shared/aggregation';
import { ChangeActiveAggregationBuckets, ChangeActiveAggregationBucketList } from '../../../state/search.state';
import { Observable } from 'rxjs';
import { NG_VALUE_ACCESSOR } from '@angular/forms';
import { CheckboxHierarchyDTO } from '../../checkboxHierarchy-dto';
...
export class FilterBoxComponent implements OnInit {
  ...
  filterItemChanged(active: boolean, bucket: AggregationBucket) {
    if (this.aggregation) {
      this.store.dispatch(new ChangeActiveAggregationBuckets(this.aggregation, bucket, active));
    }
  }
  ...
  filterHierarchyItemsChanged(bucketIds: string[]) {
    ...
    if (this.aggregation) {
      this.store.dispatch(new ChangeActiveAggregationBucketList(this.aggregation, bucketNames));
    }
  }
  ...
  filterItemsChanged(buckets: string[]) {
    if (this.aggregation) {
      this.store.dispatch(new ChangeActiveAggregationBucketList(this.aggregation, buckets));
    }
  }

  multiSelectFilterItemsChanged(buckets: [{ key: string, doc_count: number }]) {
    if (this.aggregation) {
      this.store.dispatch(new ChangeActiveAggregationBucketList(this.aggregation, buckets.map(b => b.key)));
    }
  }
}
