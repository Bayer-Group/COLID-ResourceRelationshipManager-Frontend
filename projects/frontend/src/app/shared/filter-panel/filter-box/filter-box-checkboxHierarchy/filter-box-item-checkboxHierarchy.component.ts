...
import { DisableResourceTypeItem, EnableResourceTypeItem, FetchResourceTypeHierarchy, MetadataState, ToggleCategoryFilterTab, ToggleResourceTypeItem } from '../../../../state/metadata.state';
...
export class FilterBoxItemCheckboxHierarchyComponent implements OnInit, OnDestroy {
  @Select(MetadataState.getMetadata) metadata$: Observable<any>;
  @Select(MetadataState.getResourceTypeHierarchy) resourceHierarchy$: Observable<CheckboxHierarchyDTO[]>;//statehierarchy from metadata
  ...
  ngOnInit() {
    this.masterSub.add(this.activeCategoryTab$.subscribe(x => {
      this.activeTab = x;
    }));
    this.masterSub.add(this.activeNodes$.subscribe(x => {
      this.activeNodes = x;
    }));
    this.masterSub.add(this.resourceHierarchy$.subscribe(r => {
      if (r) {
  ...
  hasChild = (_: number, _nodeData: CheckboxHierarchyDTO) => {
    return _nodeData.children.length; //changed the return
  }
